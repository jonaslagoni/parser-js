...
function assignNameToAnonymousMessages(doc) {
   ...
   */
  const addNameToKey = (messages, channelName) => {
    messages.forEach(m => {
      if (m.name() === undefined) {
        m.json()[String(xParserMessageName)] = channelName;
      }
    });
  };
...
function traverseSchema(schema, callback, prop, schemaTypesToIterate) {
  ...
  if (schemaTypesToIterate && !schemaTypesToIterate.includes(SchemaTypesToIterate.arrays) && schema.type() === 'array') return;
  if (schemaTypesToIterate && !schemaTypesToIterate.includes(SchemaTypesToIterate.objects) && schema.type() === 'object') return;
...
function traverseCombinedSchemas(schema, callback, schemaTypesToIterate) {
  ...
  if (!schemaTypesToIterate || schemaTypesToIterate.includes(SchemaTypesToIterate.allOfs)) {
    checkCombiningSchemas(schema.allOf());
  }
  if (!schemaTypesToIterate || schemaTypesToIterate.includes(SchemaTypesToIterate.anyOfs)) {
    checkCombiningSchemas(schema.anyOf());
  }
  if (!schemaTypesToIterate || schemaTypesToIterate.includes(SchemaTypesToIterate.oneOfs)) {
...
function traverseMessage(message, callback, schemaTypesToIterate) {
  if (!schemaTypesToIterate || schemaTypesToIterate.includes(SchemaTypesToIterate.headers)) {
    traverseSchema(message.headers(), callback, null, schemaTypesToIterate);
  }
  if (!schemaTypesToIterate || schemaTypesToIterate.includes(SchemaTypesToIterate.payloads)) {
